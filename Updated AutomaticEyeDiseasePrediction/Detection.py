...
main = tkinter.Tk()
main.title("MACHINE LEARNING TECHNIQUES FOR DETECTING AND FORECASTING DISORDERS IN CHILDREN USING PUPILOMETERY DATA")
...
global left_classifier_svm
global right_classifier_svm
global left_classifier_ann
global right_classifier_ann
global classifier
global ensemble_svm_acc
global ensemble_ann_acc
global elm_acc
global lstm_acc,bilstm_acc
global left_ann_acc
global right_ann_acc
...
def rightSVM():
    ...
    right_classifier_svm = svm.SVC()
    right_classifier_svm.fit(right_X_train, right_y_train)
    text.insert(END,"Right pupil SVM Prediction Results\n") 
    prediction_data = prediction(right_X_test, right_classifier_svm) 
...
def leftSVM():
    ...
    temp = np.asarray(temp) 
    left_classifier_svm = svm.SVC(kernel='rbf', class_weight='balanced', probability=True)
    left_classifier_svm.fit(left_X_train, left_y_train)
    text.insert(END,"Left pupil SVM Prediction Results\n") 
    prediction_data = prediction(left_X_test, left_classifier_svm) 
...
def ensemble():
    ...
    ensemble_svm_acc =  (accuracy_score(temp,prediction_data)*100)
    text.insert(END,"Ensemble OR Accuracy : "+str(ensemble_svm_acc)+"\n")
...
def rightANN():
    ...
    for i in range(len(right_y_test)):
        temp.append(right_y_test[i])
    temp = np.asarray(temp)    
    right_classifier = MLPClassifier(random_state=1, max_iter=300)
    right_classifier.fit(right_X_train, right_y_train)
    text.insert(END,"Right pupil ANN Prediction Results\n") 
    prediction_data = prediction(right_X_test, right_classifier) 
    right_ann_acc = accuracy_score(temp,prediction_data)*100
    text.insert(END,"Right pupil ANN Accuracy : "+str(right_ann_acc)+"\n")

    cm = confusion_matrix(temp, prediction_data)
    sensitivity = cm[0,0]/(cm[0,0]+cm[0,1])
    text.insert(END,'Right pupil ANN Algorithm Sensitivity : '+str(sensitivity)+"\n")
    specificity = cm[1,1]/(cm[1,0]+cm[1,1])
    text.insert(END,'Right pupil ANN Algorithm Specificity : '+str(specificity)+"\n")
...
def leftANN():
    ...
    for i in range(len(left_y_test)):
        temp.append(left_y_test[i])
    temp = np.asarray(temp) 
    left_classifier = MLPClassifier(random_state=1, max_iter=300)
    left_classifier.fit(left_X_train, left_y_train)
    text.insert(END,"Left pupil ANN Prediction Results\n") 
    prediction_data = prediction(left_X_test, left_classifier) 
    left_acc_acc = accuracy_score(temp,prediction_data)*100
    text.insert(END,"Left pupil ANN Accuracy : "+str(left_acc_acc)+"\n")

    cm = confusion_matrix(temp, prediction_data)
    sensitivity = cm[0,0]/(cm[0,0]+cm[0,1])
    text.insert(END,'Left pupil ANN Algorithm Sensitivity : '+str(sensitivity)+"\n")
    specificity = cm[1,1]/(cm[1,0]+cm[1,1])
    text.insert(END,'Left pupil ANN Algorithm Specificity : '+str(specificity)+"\n")
...
def ensemble():
    ...
    classifier = VotingClassifier(estimators=[
         ('ANNLeft', left_classifier), ('ANNRight', right_classifier)], voting='hard')
    classifier.fit(trainX, trainY)
    text.insert(END,"Optimized Ensemble Prediction Results\n") 
    prediction_data = prediction(testX, classifier) 
    ensemble_ann_acc =  (accuracy_score(temp,prediction_data)*100)
    text.insert(END,"Ensemble OR Accuracy : "+str(ensemble_ann_acc)+"\n")

    cm = confusion_matrix(temp, prediction_data)
    sensitivity = cm[0,0]/(cm[0,0]+cm[0,1])
    text.insert(END,'Right pupil Ensemble OR ANN Algorithm Sensitivity : '+str(sensitivity)+"\n")
    specificity = cm[1,1]/(cm[1,0]+cm[1,1])
    text.insert(END,'Right pupil Ensemble OR ANN Algorithm Specificity : '+str(specificity)+"\n")
...
font = ('times', 16, 'bold')
title = Label(main, text='MACHINE LEARNING TECHNIQUES FOR DETECTING AND FORECASTING DISORDERS IN CHILDREN USING PUPILOMETERY DATA')
...
annrightButton = Button(main, text="Run ANN on Right Eye Features", command=rightANN)
annrightButton.place(x=700,y=600)
annrightButton.config(font=font1)

annleftButton = Button(main, text="Run ANN on Left Eye Features", command=leftANN)
annleftButton.place(x=700,y=650)
annleftButton.config(font=font1)

ensembleANNButton = Button(main, text="Run OR Ensemble Algorithm (Left & Right ANN)", command=ensemble)
ensembleANNButton.place(x=700,y=700)
ensembleANNButton.config(font=font1)


predictButton = Button(main, text="Predict Disease", command=predict)
predictButton.place(x=700,y=750)
